...
from cac_dataset_mood import CACTwoClassDataset
...
from resnet_mood import ClsResNet
...
def main(save_path=cfg.save,      # configuration file
    ...
    test_set = CACTwoClassDataset(crop_size=[48,48,48], data_path=env.data, datatype=1, fill_with=-1)
    ...
    train(model=model, train_set=train_set, test_set=test_set, save=save_path, n_epochs=n_epochs)

    print('Done!')



def train(model, train_set, test_set, save, n_epochs):
    '''
    ...
    for epoch in range(n_epochs):
        ...
        scheduler.step(log_dict['test_loss'])
        
        # if cfg.save_all:
        if log_dict['test_auc']>0.8:
